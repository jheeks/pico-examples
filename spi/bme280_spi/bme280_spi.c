...
static uint8_t* read_registers(uint8_t reg, uint16_t len) {
    // For this particular device, we send the device the register we want to read
    // first, and subsequently read from the device. The register is auto incrementing
    // so we don't need to keep sending the register we want, just the first.
    // This is kept together in one spi call to get the chip select controlled correctly.
    txBuf[0] = reg | READ_BIT;
    spi_write_read_blocking(spi_default, txBuf, rxBuf, len+1);
    return &rxBuf[1];
}
...
#warning spi/bme280_spi example requires a board with SPI pins
    ...
    // chip select is handled by SPI HW block in case of CPOL=1, CPHA=1
    ...
    uint8_t* buffer_id;
    buffer_id = read_registers(0xD0, 1);
